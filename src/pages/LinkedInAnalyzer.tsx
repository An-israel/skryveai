            ...
            <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-6">
                    ...
                    <div className="text-center sm:text-left space-y-2 flex-1">
                      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                        ...
                        <Badge variant={result.profileStrength === "All-Star" || result.profileStrength === "Expert" ? "default" : "secondary"} className="text-sm px-3">
                          {result.profileStrength}
                        </Badge>
                      </div>
                      <p className="text-muted-foreground text-sm max-w-md">
                        Your LinkedIn profile scores <strong>{result.overallScore}/100</strong>.{" "}
                        {result.overallScore < 60 && "Several key improvements can significantly boost your visibility."}
                        {result.overallScore >= 60 && result.overallScore < 80 && "On the right track — targeted improvements will make a real difference."}
                        {result.overallScore >= 80 && "Strong profile! A few final tweaks to reach All-Star status."}
                      </p>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {(Object.keys(result.breakdown) as (keyof SectionBreakdown)[]).map((key) => {
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {result.biggerImprovements.map((item, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Card className="border-primary/20 bg-primary/5">
                  ...
                  <CardContent className="space-y-3">
                    <p className="text-sm font-medium leading-snug">{result.headlineSuggestion}</p>
                    <Button variant="outline" size="sm" className="gap-2 w-full" onClick={() => copyText(result.headlineSuggestion, "headline")}>
                ...
                <Card className="border-primary/20 bg-primary/5">
                  ...
                  <CardContent className="space-y-3">
                    <p className="text-sm leading-relaxed">{result.aboutSuggestion}</p>
                    <Button variant="outline" size="sm" className="gap-2 w-full" onClick={() => copyText(result.aboutSuggestion, "about")}>
            ...
            <motion.div key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
                ...
                <CardContent className="p-4 flex gap-3">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <p className="text-muted-foreground">
                      On LinkedIn, go to your profile → click <strong>More</strong> → <strong>Save to PDF</strong>. Upload that PDF below — it contains everything needed to analyze your profile.
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="target-role">
                  Target Role <span className="text-muted-foreground text-xs">(optional — makes analysis more specific)</span>
                </Label>
                <Input
                  id="target-role"
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <Label htmlFor="profile-content" className="text-muted-foreground">
