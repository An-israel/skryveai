...
serve(async (req) => {
  ...
  try {
    ...
    const gate = await enforceToolLimit(serviceClient, user.id, mode === "linkedin" ? "linkedin" : "ats_checker");
...
- Only add an element to missingElements if there is genuinely zero content of that type anywhere in the profile text.`,
      ...
      if (!toolUse?.input) throw new Error("No analysis returned from AI");
